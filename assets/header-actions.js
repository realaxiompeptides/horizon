...
class HeaderActions extends Component {
  ...
  /**
   * Syncs aria-expanded on the cart drawer trigger
  ...
  /**
   * Handles cart update events and announces
   * the new cart count to screen readers.
   *
    ...
    event.promise
      ...
      .catch((error) => {
        if (error?.name !== 'AbortError') {
          console.warn(
            '[header-actions] Event promise rejected:',
            error
          );
        }
      });
